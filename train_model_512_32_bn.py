...
model_name = str(filters) + '_filters_bn_batch_size_' + str(batch_size) + '_test_' + str(test_nr) 
# folders
...
for result in preds_test:
    ...
    img_erosion = cv.erode(img_squeezed_t,kernel,iterations = 1)
    img_opening = cv.dilate(img_erosion,kernel,iterations = 1)
    ...
    if not os.path.isdir(output_foldername+ '/results_t_opening'):
        os.mkdir(output_foldername + '/results_t_opening')    
    
    result_path = output_foldername + '/results/' + str(counter) + '.PNG'
    result_path_t = output_foldername + '/results_t/' + str(counter) + '.PNG'
    result_path_t_opening = output_foldername + '/results_t_opening/' + str(counter) + '.PNG'

    cv.imwrite(result_path, img_squeezed)
    cv.imwrite(result_path_t, img_squeezed_t)
    cv.imwrite(result_path_t_opening, img_opening)
...
for result in preds_test2:
    ...
    # Erosion
    kernel = np.ones((3,3),np.uint8) # 3x3 Filter
    img_erosion2 = cv.erode(img_squeezed_t2,kernel,iterations = 1)
    # Dilation
    img_opening2 = cv.dilate(img_erosion2,kernel,iterations = 1)
    ...
    if not os.path.isdir(output_foldername2 + '/results_t_opening'):
        os.mkdir(output_foldername2 + '/results_t_opening')    
    
    result_path2 = output_foldername2 + '/results/' + str(counter) + '.PNG'
    result_path_t2 = output_foldername2 + '/results_t/' + str(counter) + '.PNG'
    result_path_t_opening2 = output_foldername2 + '/results_t_opening/' + str(counter) + '.PNG'

    cv.imwrite(result_path2, img_squeezed2)
    cv.imwrite(result_path_t2, img_squeezed_t2)
    cv.imwrite(result_path_t_opening2, img_opening2)
