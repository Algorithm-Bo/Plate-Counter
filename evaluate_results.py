...
# configure folders
# binary result image
# input_name = '32_filters_batch_size_4_bn_vali_extra_neu_2'
input_name = '32_filters_3_blocks_batch_size_4_test_1'

# data set 2
input_name2 = input_name + '_examples'

input_foldername = 'results/' + input_name + '/results_t'
input_foldername2 = 'results/' + input_name2 + '/results_t'

output_foldername = 'connected_components_images/' + input_name
output_foldername2 = 'connected_components_images/' + input_name2

output_foldername_CSV = 'CSV_lists/' + input_name
output_foldername_CSV2 = 'CSV_lists/' + input_name2

json_output_foldername = 'JSON_data/' + input_name
json_output_foldername2 = 'JSON_data/' + input_name2

# test images folder
image_folder = 'data/images_test_staphylococcus_512x512_20'
image_folder2 = 'data/images_test_examples'

json_folder = json_output_foldername
json_folder2 = json_output_foldername2

output_foldername_bb = 'bounding_boxes_images/' + input_name
output_foldername_bb2 = 'bounding_boxes_images/' + input_name2
...
filename_list2 = os.listdir(input_foldername2)
# print(filename_list)
...
if not os.path.isdir(output_foldername_CSV2):
    os.mkdir(output_foldername_CSV2)

if not os.path.isdir(json_output_foldername):
    os.mkdir(json_output_foldername)

if not os.path.isdir(json_output_foldername2):
    os.mkdir(json_output_foldername2)

if not os.path.isdir(output_foldername_bb):
    os.mkdir(output_foldername_bb)

if not os.path.isdir(output_foldername_bb2):
    os.mkdir(output_foldername_bb2)
...
for image in range(0, len(filename_list)):
    ...
    for coord in range(0, n_labels):
        ...
        # area = sizes[label]
        position_data_list = []
        position_data_list = [pos_x, pos_y, x_upper_left, y_upper_left, width, height]
...
object_count_list2 = []
for image in range(0, len(filename_list2)):
    filename = str(image) + '.PNG'
    path2 = input_foldername2 + '/' + filename

    output_filename_color = filename
    output_path_color2 = output_foldername2 + '/' + output_filename_color
    ...
    for coord in range(0, n_labels):
        ...
        dict_coordinates2[object_count] = position_data_list
        
    # append list
    object_count_list2.append(object_count)

    # print('Colonies detected: ', object_count)
    # print('dict coordinates2')
    # pprint(dict_coordinates2)

    # Create JSON Dictionary
    dict_coordinates_json2 = json.dumps(dict_coordinates2, indent = 2, sort_keys=True)

    # write in file
    json_path2 = json_output_foldername2 + '/' + filename[:-4] + '.json'

    with open(json_path2,'w', encoding = 'utf-8') as file:
        # file.write(filename)
        file.write(dict_coordinates_json2)
        
# Create CSV Table

CSV_path2 = output_foldername_CSV2 + '/' + 'Counted_Objects.CSV'

counter = 0

with open(CSV_path2, 'w') as fp:
    for elem in object_count_list2:
        string1 = 'Image ' + str(counter) + '.PNG' 
        fp.write(string1)
        fp.write(';')
        fp.write(str(elem))
...
filename_list2 = os.listdir(image_folder2)
print(filename_list2)

for image in range(0, len(filename_list2)):
    filename = str(image) + '.PNG'

    image_path2 = image_folder2 + '/' + filename

    output_filename = filename
    
    output_path_bb2 = output_foldername_bb2 + '/' + output_filename

    # print(image_path)
    # JSON Koordinaten lesen und in Bild darstellen

    json_filename = filename[:-4] + '.JSON'
    json_path2 = json_folder2 + '/' + json_filename

    if os.path.isfile(json_path2):
        with open(json_path2, "r") as read_file:
            data = json.load(read_file)
    ...
    cv2.imshow('Image with Bounding Boxes', img_with_bb)
    cv2.waitKey(0)
    cv2.imwrite(output_path_bb2, img_with_bb)
